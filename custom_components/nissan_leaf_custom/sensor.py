...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    for vin, datastore in hass.data[DATA_LEAF].items():
        ...
        devices.append(LeafBatteryBarSensor(datastore))
        devices.append(LeafBatteryBarCapacitySensor(datastore))
...
class LeafBatteryBarSensor(LeafEntity, SensorEntity):
    """Nissan Leaf Bar Sensor."""

    @property
    def name(self):
        """Sensor Name."""
        return f"{self.car.leaf.nickname} Battery Bars"

    @property
    def state(self):
        """Battery state percentage."""
        return round(self.car.data[DATA_BATTERY_REMAINING])
...
class LeafBatteryBarCapacitySensor(LeafEntity, SensorEntity):
    """Nissan Leaf Capacity Sensor."""

    @property
    def name(self):
        """Sensor Name."""
        return f"{self.car.leaf.nickname} Battery Capacity"

    @property
    def state(self):
        """Battery state percentage."""
        return round(self.car.data[DATA_BATTERY_CAPACITY])
